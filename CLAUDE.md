...
- Pages: `index.mdx` (splash home), `spec.md`, `demo.mdx`, `diagrams/{index,create,resolve,update,beacons,data}.md`, the use case group `diagrams/stuart-meets-satoshi/{index,key-rotation,states,data}.mdx`, `parity.md`, `impls.md`, `impls/{java,py,rs}.md`, and the TypeScript group `impls/ts/{index.mdx,sdk.mdx,cli.mdx}` (Overview, SDK, CLI).
