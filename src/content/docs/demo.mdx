...
The form resolves with `minConf` 1: the update must build on the version of
your last update. With the resolution default (6), the resolution misses an
update with fewer than 6 confirmations. The new update then targets the same
`versionId`, and resolution of the DID fails with `LATE_PUBLISHING_ERROR`.
